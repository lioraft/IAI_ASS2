...
def play_with_advanced_heuristics():
    ...
    player_1 = player_agent_heuristics(heuristic_alpha_beta_isolation.alphabeta_max_h, heuristics.advanced_heuristic,
                                       depth_player_1)
    player_2 = player_agent_heuristics(heuristic_alpha_beta_isolation.alphabeta_min_h, heuristics.advanced_heuristic,
                                       depth_player_2)
    grid = np.zeros((7, 7), dtype=int)
    init_state = game_state(grid, (0, 0), (3, 3), 1)
    play_isolation(player_1, player_2, init_state)

def play_with_combined_heuristics():
    depth_player_1 = 7
    depth_player_2 = 7
    #Does player 1 wins most of the games you play?
    player_1 = player_agent_heuristics(heuristic_alpha_beta_isolation.alphabeta_max_h, heuristics.combined_heuristic,
                                       depth_player_1)
    player_2 = player_agent_heuristics(heuristic_alpha_beta_isolation.alphabeta_min_h, heuristics.combined_heuristic,
                                       depth_player_2)
    grid = np.zeros((7, 7), dtype=int)
    init_state = game_state(grid, (0, 0), (3, 3), 1)
    play_isolation(player_1, player_2, init_state)
...
if __name__ == '__main__':
    ...
    play_with_combined_heuristics()
    end_time2 = time.time()
    print(f"Time for heuristics: {end_time - start_time}")
    print(f"Time for advanced heuristics: {end_time1 - start_time1}")
    print(f"Time for combined heuristics: {end_time2 - start_time2}")